...
void MyMesh::onDiscoveredContact(ContactInfo &contact, bool is_new, uint8_t path_len, const uint8_t* path) {
  ...
  if (path && path_len <= sizeof(AdvertPath::path)) {  // check path is valid
    ...
    // corrupt the advert_paths table (garbled "recently heard" entries).
...
void MyMesh::handleCmdFrame(size_t len) {
  if (cmd_frame[0] == CMD_DEVICE_QUERY && len >= 2) { // sent when app establishes connection
    ...
    // MeshCore 1.16: persistent default region scope. Payload [63, name(31), key(16)];
    // an empty payload ([63]) clears it. Without this handler the official app's
    // "default scope" experimental setting got no reply ("no_event_received").
    if (len >= 1 + 31 + 16) {
